Shader "Noriben/noribenLED_BarLight"
{
	Properties
	{
		[Header(Texture)]
		_MovieTex ("Movie RenderTex", 2D) = "white" {}
		...
		[Toggle] _VertexCol2IDToggle ("Use Vertex Color as ID", float) = 0
		...
		[Toggle(DepthFadeToggle)] _DepthFadeOn ("Soft Depth Occlusion", Float) = 1
		_DepthFade ("Depth Fade Power", Range(0.05, 4.0)) = 0.75
		_DepthFadeRootIgnore ("Depth Fade Root Ignore Range", Range(0.0, 0.5)) = 0.07

		[Space]
		[Header(AudioLink for VRChat)]
		[Toggle(AudioLinkOn)] _AudioLinkOn ("AudioLink On", Float) = 0
		
		_AudioLinkIntensity ("AudioLink Intensity", Range(0,4)) = 1
		...
		_AudioUVShift ("Audio UV Shift", float) = 0
        [HideInInspector]_WaveWidth ("Wave Width", float) = 1
        [HideInInspector]_HistoryMultiSample ("Audio History MultiSample Smoothing", Range(0, 20)) = 1
		...
		_SampleTexUVShiftX ("SamplingTex UV Shift X", float) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			float _AudioUVShift;
			float _WaveWidth;
			float _HistoryMultiSample;
			float _ObjectID;
			float _SampleTexUVShiftX;
			float _VertexCol2IDToggle;
			...
			UNITY_INSTANCING_BUFFER_START(Props)
				...
				UNITY_DEFINE_INSTANCED_PROP(float, _DepthFadeRootIgnore)
			...
			fixed4 frag (v2f i, fixed facing:VFACE) : SV_Target
			{
				...
				float DepthFadeRootIgnore = UNITY_ACCESS_INSTANCED_PROP(Props, _DepthFadeRootIgnore);
				...
				// 頂点カラーをオブジェクトの識別に使う
				float ObjectID = lerp(_ObjectID, i.color.x * 10 + 0.0001, _VertexCol2IDToggle);

				// AudioLink
				float4 AudioLinkCol = float4(1,1,1,1);
				float4 movieTex = float4(1,1,1,1);
				#ifdef AudioLinkOn
				/*
					...
					*/
					...
					/* オーディオリンクのマルチサンプリング、いらないかも
					float historyCol = 0;
					float audioUVShift = _AudioUVShift * _ObjectID;
					for (int k = 0; k <= _HistoryMultiSample; k++)
					{
						float shift = audioUVShift + 0.02 * k;
						historyCol += AudioLinkLerp(ALPASS_AUDIOLINK + float2(shift * 128 * _WaveWidth, AudioLinkType));
					}
					historyCol /= _HistoryMultiSample + 1;
					float4 audioLinkCol = float4(historyCol, historyCol, historyCol,1);
					*/

					float audioUVShift = _AudioUVShift * ObjectID;
					float audioLinkBaseCol = AudioLinkLerp(ALPASS_AUDIOLINK + float2(audioUVShift * 128 * _WaveWidth, AudioLinkType));
					float4 audioLinkCol = float4(audioLinkBaseCol, audioLinkBaseCol, audioLinkBaseCol, 1);
					
					// 動画テクスチャのサンプリング
					//float4 movieTex = tex2D(_MainTex, float2(.8, _AudioUVShift));
					float movieMultiSampling = 8;
					for (int j = 0; j <= movieMultiSampling; j++)
					{
						float uvShift = 0.04 * j;
						movieTex += tex2D(_MovieTex, float2(audioUVShift + .3, uvShift + .3));
					}
					movieTex /= movieMultiSampling + 1;

					// AudioLinkColにmovieTexの色相・彩度を反映
					float3 audioLinkColHSV = rgb2hsv(audioLinkCol.xyz);
					float3 movieTexHSV = rgb2hsv(movieTex.xyz);
					audioLinkColHSV = float3(movieTexHSV.x, movieTexHSV.y, audioLinkColHSV.z);
					audioLinkColHSV = hsv2rgb(audioLinkColHSV);
					float4 movieAudioCol = float4(audioLinkColHSV,1);

					//col = col * movieTex + (movieTex * .05);
					AudioLinkCol = movieAudioCol * AudioLinkIntensity;

					// AudioLinkのテクスチャが存在しているか判定
					...
					{
						// AudioLinkのテクスチャが存在しないときの描画

						/*
						float noAudioLinkCol = sin(_AudioUVShift * 4 + _Time.y * .5) + 1. * .5;
						noAudioLinkCol = pow(noAudioLinkCol, 2);
						noAudioLinkCol = saturate(noAudioLinkCol + 0);
						float3 noAudioLinkCol3 = float3(noAudioLinkCol, noAudioLinkCol, noAudioLinkCol);

						float3 noAudioLinkColHSV = rgb2hsv(noAudioLinkCol3);
						noAudioLinkColHSV = float3(movieTexHSV.x, movieTexHSV.y, noAudioLinkColHSV.z);
						float3 noAudioLinkColRGB = hsv2rgb(noAudioLinkColHSV);
						float4 movieNoAudioCol = float4(noAudioLinkColRGB,1);


						AudioLinkCol = saturate(movieNoAudioCol);
						
						*/
						AudioLinkCol = (float4)1;
				...
				//端のだんだん消えていく感じのマスク
				float mask = smoothstep(0.01, 1, distance(i.uv.y, 0.0));
				//リムライト的処理エッジをやわらかく消す
				...
				float sampleTexUV_ShiftX = _SampleTexUVShiftX * ObjectID;
				sampledColor = tex2D(_SampleTex, float2(SamplePosX + sampleScrollSpeed + sampleTexUV_ShiftX, SamplePosY));
				// 3つのUVポジションからサンプルし平均を取る
				float4 sampledColor1 = tex2D(_SampleTex, float2(SamplePosX - .01 + sampleScrollSpeed + sampleTexUV_ShiftX, SamplePosY - .04));
				float4 sampledColor2 = tex2D(_SampleTex, float2(SamplePosX + .02 + sampleScrollSpeed + sampleTexUV_ShiftX, SamplePosY - .02));
				float4 sampledColor3 = tex2D(_SampleTex, float2(SamplePosX - .03 + sampleScrollSpeed + sampleTexUV_ShiftX, SamplePosY + .03));
				float4 sampledColor4 = tex2D(_SampleTex, float2(SamplePosX + .04 + sampleScrollSpeed + sampleTexUV_ShiftX, SamplePosY + .01));
				...
				/*
				// AudioLinkが動いているときはsampledColorを無視する
				#ifdef AudioLinkOn
					// AudioLinkのテクスチャが存在しているか判定
					if (_AudioTexture_TexelSize.z > 16)
					{
						// AudioLinkのテクスチャが存在しているときの処理
						sampledColor = float4(1,1,1,1);
					}
					else
					{
						// AudioLinkのテクスチャが無いときの処理
				...
				*/
				...
				float randVal = random1D(LightIndex + 1 + ObjectID); // 適当なランダム値				
				...
				/*
				// AudioLinkが動いているときはColorを無視する
				#ifdef AudioLinkOn
					// AudioLinkのテクスチャが存在しているか判定
					if (_AudioTexture_TexelSize.z > 16)
					{
						desaturatedColor = float3(1,1,1);
					}
				#endif
				*/
				float4 col = float4(baseCol, baseCol, baseCol, 1) * float4(desaturatedColor.xyz, 1) * noiseTex * sampledColor * AudioLinkCol;


				//レンズ部分の色
				float baselensColor = grad * Intensity * pow(1.5, - ConeWidth * 20);
				float3 lensColor = float3(baselensColor, baselensColor, baselensColor) * desaturatedColor.xyz * sampledColor * AudioLinkCol;
				...
				col = clamp(col,0,maxIntensity) * Opacity;
				...
				// 床面を照らす用カラー（フロントメッシュ）
				float4 frontCol = float4(desaturatedColor.xyz, 1) * sampledColor * AudioLinkCol * rimFront * Intensity * grad * mask * noiseTex * .6; // 1は明るさ調整
				frontCol *= float4(worldHeight, worldHeight, worldHeight, 1);
				frontCol = clamp(frontCol, 0, maxIntensity) * Opacity;
				
				// movieTexの色をclamp後の色に反映(レンズ部分まわりに色をつけるため)
				//単に乗算するとmovieTexが黒いときに表示されなくなってしまうので白とのlerpを取る
				float4 movieSubCol = lerp(movieTex, float4(1,1,1,1), .2);
				frontCol *= movieSubCol;

				float4 backCol = col * movieSubCol;

				col = facing > 0 ? frontCol : backCol;

				#ifdef DepthFadeToggle
					float4 depthSample = SAMPLE_DEPTH_TEXTURE_PROJ(
						_CameraDepthTexture,
						UNITY_PROJ_COORD(i.screenPos));
					half sceneDepth = LinearEyeDepth(depthSample);
					half softOcclusion = saturate(sceneDepth - i.screenPos.w);
					float missingCameraDepth = 0;

					// VRChatのScreen CameraにDepth Textureが無い場合は従来描画へ戻す。
					if (softOcclusion < 0.00001)
					{
						missingCameraDepth = 1;
						softOcclusion = 0;
					}

					softOcclusion = pow(softOcclusion, DepthFade);

					// BeamLightと同じ考え方で、機械部分に接する根本だけ
					// Depth Fadeの影響を滑らかに打ち消す。
					float rootDistance = saturate(1.0 - i.uv.y);
					float rootIgnoreRange = max(DepthFadeRootIgnore, 0.0001);
					float rootIgnoreDepth = saturate(
						smoothstep(0.0, rootIgnoreRange, rootDistance) - 0.001);
					rootIgnoreDepth = 1.0 - pow(rootIgnoreDepth, 0.1);
					rootIgnoreDepth *= step(0.0001, DepthFadeRootIgnore);
					softOcclusion = saturate(softOcclusion + rootIgnoreDepth);

					if (missingCameraDepth < 0.99)
					{
						col *= float4(softOcclusion.xxx, 1);
					}
				#endif
